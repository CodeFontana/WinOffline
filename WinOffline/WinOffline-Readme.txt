Purpose:
WinOffline is a utility designed to simplify IT Client Manager (ITCM) product patching and automate common maintenance procedures.
...
VERSIONCHECK -- Designate patch is ITCM version specific. The patch will be SKIPPED if the version does not match.
...
DEPENDS -- Designate external file dependencies. Useful when delivering scripts to be run by WinOffline that have dependent files.
